from typing import Any, Dict, List

import pytest

from src.generators import card_number_generator, filter_by_currency, transaction_descriptions


@pytest.fixture
def transactions() -> List[Dict[str, Any]]:
    return [
        {
            "id": 939719570,
            "state": "EXECUTED",
            "date": "2018-06-30T02:08:58.425572",
            "operationAmount": {"amount": "9824.07", "currency": {"name": "USD", "code": "USD"}},
            "description": "Перевод организации",
            "from": "Счет 75106830613657916952",
            "to": "Счет 11776614605963066702",
        },
        {
            "id": 142264268,
            "state": "EXECUTED",
            "date": "2019-04-04T23:20:05.206878",
            "operationAmount": {"amount": "79114.93", "currency": {"name": "USD", "code": "USD"}},
            "description": "Перевод со счета на счет",
            "from": "Счет 19708645243227258542",
            "to": "Счет 75651667383060284188",
        },
        {
            "id": 873106923,
            "state": "EXECUTED",
            "date": "2019-03-23T01:09:46.296404",
            "operationAmount": {"amount": "43318.34", "currency": {"name": "руб.", "code": "RUB"}},
            "description": "Перевод со счета на счет",
            "from": "Счет 44812258784861134719",
            "to": "Счет 74489636417521191160",
        },
        {
            "id": 895315941,
            "state": "EXECUTED",
            "date": "2018-08-19T04:27:37.904916",
            "operationAmount": {"amount": "56883.54", "currency": {"name": "USD", "code": "USD"}},
            "description": "Перевод с карты на карту",
            "from": "Visa Classic 6831982476737658",
            "to": "Visa Platinum 8990922113665229",
        },
        {
            "id": 594226727,
            "state": "CANCELED",
            "date": "2018-09-12T21:27:25.241689",
            "operationAmount": {"amount": "67314.70", "currency": {"name": "руб.", "code": "RUB"}},
            "description": "Перевод организации",
            "from": "Visa Platinum 1246377376343588",
            "to": "Счет 14211924144426031657",
        },
    ]


@pytest.mark.parametrize(
    "currency_code, expected_count, expected_ids",
    [
        ("USD", 3, [939719570, 142264268, 895315941]),  # Валюта, ожидаемое количество, ожидаемые ID
        ("RUB", 2, [873106923, 594226727]),
        ("EUR", 0, []),  # Валюта, которой нет
        ("USD", 3, [939719570, 142264268, 895315941]),  # Дублируем, проверяем, что работает
    ],
)
def test_filter_by_currency_parametrized(
    transactions: List[Dict[str, Any]],
    currency_code: str,
    expected_count: int,
    expected_ids: List[int],  # Теперь проверяем ID
) -> None:
    """
    Тестирует с разными валютами и проверяет количество и ID транзакций.
    """
    filtered_gen = filter_by_currency(transactions, currency_code)
    filtered_list = list(filtered_gen)
    assert len(filtered_list) == expected_count  # Проверяем количество

    actual_ids = [tx.get("id") for tx in filtered_list]
    assert actual_ids == expected_ids  # Проверяем ID

    # Проверяем, что у всех полученных транзакций правильный код валюты
    if filtered_list:  # если список не пустой
        for tx in filtered_list:
            assert tx.get("operationAmount", {}).get("currency", {}).get("code") == currency_code


def test_filter_by_currency_empty_input_list() -> None:
    """Тестирует работу с пустым списком транзакций."""
    filtered_gen = filter_by_currency([], "USD")
    assert list(filtered_gen) == []


def test_transaction_descriptions_correct_order(transactions: List[Dict[str, Any]]) -> None:
    """Проверяет, что генератор выдает описания в правильном порядке."""
    descriptions_gen = transaction_descriptions(transactions)
    expected_descriptions = [
        "Перевод организации",
        "Перевод со счета на счет",
        "Перевод со счета на счет",
        "Перевод с карты на карту",
        "Перевод организации",
    ]
    assert list(descriptions_gen) == expected_descriptions


def test_transaction_descriptions_empty_list() -> None:
    """Проверяет, что генератор корректно обрабатывает пустой список транзакций."""
    descriptions_gen = transaction_descriptions([])
    assert list(descriptions_gen) == []


def test_transaction_descriptions_no_description_key() -> None:
    """Проверяет, что транзакции без ключа 'description' игнорируются."""
    transactions_with_missing_desc: List[Dict[str, Any]] = [
        {"id": 1, "description": "Оплата"},
        {"id": 2, "amount": 100},  # Нет ключа "description"
        {"id": 3, "description": "Покупка"},
    ]
    descriptions_gen = transaction_descriptions(transactions_with_missing_desc)
    expected_descriptions = ["Оплата", "Покупка"]
    assert list(descriptions_gen) == expected_descriptions


@pytest.mark.parametrize(
    "start, stop, expected_cards",
    [
        (1, 3, ["0000 0000 0000 0001", "0000 0000 0000 0002", "0000 0000 0000 0003"]),
        (10, 11, ["0000 0000 0000 0010", "0000 0000 0000 0011"]),
        (123456789012345, 123456789012345, ["0123 4567 8901 2345"]),  # Большое число
        (9999999999999998, 9999999999999999, ["9999 9999 9999 9998", "9999 9999 9999 9999"]),  # Крайние значения
        (0, 0, ["0000 0000 0000 0000"]),  # Проверка на 0
    ],
)
def test_card_number_generator_range_and_format(start: int, stop: int, expected_cards: List[str]) -> None:
    """Проверяет генерацию номеров карт в заданном диапазоне и их формат."""
    generated_cards = list(card_number_generator(start, stop))
    assert generated_cards == expected_cards
