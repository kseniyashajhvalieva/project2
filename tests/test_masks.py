...
def test_get_mask_card_number_only_four_digits(card_number_data):
...
def test_get_mask_card_number_invalid_type(card_number_data):
...
def test_get_mask_account_invalid_type(card_number_data):
    # Ожидается TypeError, так как функция принимает int, а передается str
    with pytest.raises(TypeError):
