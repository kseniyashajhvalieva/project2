...
def get_mask_card_number(card_number: int) -> str:
    ...
    if not isinstance(card_number, int):
        logger_masks.error(
            f"В функцию get_mask_card_number передан нечисловой аргумент: {type(card_number)}."
        )
    ...
    if not card_number_str.isdigit() or len(card_number_str) != 16:
        logger_masks.error(
            f"Некорректный номер карты: '{card_number_str}'. Ожидается 16 цифр."
        )
...
def get_mask_account(account_number: int) -> str:
    ...
    if not isinstance(account_number, int):
        logger_masks.error(
            f"В функцию get_mask_account передан нечисловой аргумент: {type(account_number)}."
        )
